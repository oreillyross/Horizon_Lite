...
export async function ingestAcled() {
  console.log("Running ACLED ingestion...");
  ...
  console.log(`Fetching ACLED events from ${startStr} to ${endStr}`);
  ...
  while (true) {
    const response = await fetchAcledPage(apiKey, email, page, startStr, endStr);
    ...
    console.log(
      `ACLED page ${page}: fetched=${events.length} total_upserted=${totalUpserted}`,
    );
  ...
  `);
  console.log(
    `ACLED signal expiry cleanup: ${(expiredResult as any).rowCount ?? 0} rows expired`,
  );

  console.log(
    `ACLED ingestion complete ✅ fetched=${totalFetched} upserted=${totalUpserted}`,
  );
