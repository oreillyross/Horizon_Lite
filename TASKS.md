...
- [x] Audit `HorizonUpdatesScreen` — confirm it renders the real (possibly empty) updates list gracefully; remove any hardcoded sample entries
- [x] Audit `HorizonReportsScreen` — confirm it renders the real sparse report skeleton; remove any hardcoded brief text
...
- [x] Build a suggestion review panel on `HorizonIndicatorDetailScreen` — lists pending signal events with approve / dismiss buttons
...
- [x] Implement the second ingest adapter in `server/ingest/` following the same interface and error-handling pattern as `gdeltIngest.ts`
- [x] Wire the adapter into `generateSignals.ts` so its events flow through the same confidence scoring and deduplication logic
- [x] Add an enable/disable toggle for the second feed in `AdminScreen` backed by a config row in the database (or an env var if simpler)
